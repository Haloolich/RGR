...
namespace RGR_Sabelnikova
{
    public partial class Form2 : Form
    {
        ...
        int[,] currentStateWolf = new int[mapSize, mapSize];
        int[,] nextStateWolf = new int[mapSize, mapSize];
        //клітинка-кнопка
        ...
        public void Init()
        {
            ...
            nextState = InitMap();
            currentStateWolf = InitMap();
            nextStateWolf = InitMap();
            InitCells();
        }
        ...
        bool CheckGenerationDead()
        {
            for (int i = 0; i < mapSize; i++)
            {
                ...
                {
                    if (currentState[i, j] == 1 || currentStateWolf[i,j] == 2)
                        return false;
                }
            }
            return true;
        ...
        void CalculateNextState()
        {
            for (int i = 0; i < mapSize; i++)
            {
                for (int j = 0; j < mapSize; j++)
                {
                    ...
                    {
                        nextState[tempI, tempJ] = 1;
                        nextStateWolf[tempI, tempJ] = 2;
                    }
                }
            }
            currentState = nextState;
            nextState = InitMap();
            currentStateWolf = nextStateWolf;
            nextStateWolf = InitMap();
        }
        //підрахунок сусідів
        ...
        {
            var pressedButton = sender as Button;
            if (!isPlaying && e.KeyChar == 'v')
            {
                var i = (pressedButton.Location.Y - offset) / cellSize;
                var j = pressedButton.Location.X / cellSize;

                if (currentStateWolf[i, j] == 0)
                {
                    currentStateWolf[i, j] = 2;
                    cells[i, j].BackColor = Color.Red;
                }
                else
                {
                    currentStateWolf[i, j] = 0;
                    cells[i, j].BackColor = Color.White;
                }
            }
        }
